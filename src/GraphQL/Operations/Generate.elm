...
-}
decodeHelpers : List Elm.Declaration
decodeHelpers =
    [ Elm.fn3
        ( "name", Just Type.string )
        ( "new", Just (Decode.annotation_.decoder (Type.var "a")) )
        ( "build", Just (Decode.annotation_.decoder (Type.function [ Type.var "a" ] (Type.var "b"))) )
        (\name new build ->
            build
                |> Decode.map2
                    (\_ _ ->
                ...
                |> Elm.withType (Decode.annotation_.decoder (Type.var "b"))
        )
        |> Elm.declaration "field"
    , Elm.fn2
        ( "new", Just (Decode.annotation_.decoder (Type.var "a")) )
        ( "build", Just (Decode.annotation_.decoder (Type.function [ Type.var "a" ] (Type.var "b"))) )
        (\new build ->
