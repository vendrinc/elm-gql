...
generateDefinition { namespace, schema, document, path, gqlDir } ((Can.Operation op) as def) =
    let
        ...
        fragmentDecoders =
            case frags of
                ...
                _ ->
                    generateFragmentDecoders namespace frags

        fragmentTypes =
            case List.concatMap (generateFragmentTypes namespace) frags of
                [] ->
                    []
    ...
    in
    Elm.fileWith (pathFromElmRootToGqlFile ++ [ Utils.String.formatTypename opName ])
        { aliases = []
        , docs =
            \docs ->
